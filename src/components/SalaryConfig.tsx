...
import { Eye, EyeOff, Save, Calculator, DollarSign, Percent, PiggyBank, ShoppingCart, Home, TrendingUp } from "lucide-react";
...
const SalaryConfig = ({ onSalaryUpdate, currentSalary = 0, currentBudgetPercentage = 70 }: SalaryConfigProps) => {
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [password, setPassword] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [actualSalary, setActualSalary] = useState(currentSalary);
  const [budgetPercentage, setBudgetPercentage] = useState(currentBudgetPercentage);
  ...
  // Simple password - in real app, this would be more secure
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    ...
    if (password === SECRET_PASSWORD) {
      ...
      toast({
        ...
        description: "You can now configure your salary settings.",
      });
    } else {
      toast({
        title: "Access Denied",
  ...
  const getTotalAllocationPercentage = () => {
    return budgetAllocation.need + budgetAllocation.want + budgetAllocation.savings + budgetAllocation.investments;
  };
  ...
  const getAllocatedAmounts = () => {
    return {
      ...
      investments: Math.round(calculatedBudget * budgetAllocation.investments / 100)
    };
  };
        ...
        <Card className="shadow-card">
          <CardHeader className="text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Enter password to access salary configuration
            </p>
          </CardHeader>
          <CardContent>
            ...
            <div className="mt-4 p-3 bg-muted/50 rounded-lg">
              <p className="text-xs text-muted-foreground text-center">
                Demo password: <code className="bg-background px-1 rounded">budget2024</code>
              </p>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {[50, 60, 70, 80].map((preset) => (
                <Button
                  ...
                  className={budgetPercentage === preset ? "bg-primary text-primary-foreground" : ""}
                >
                  {preset}%
                </Button>
              ))}
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                <Input
                  ...
                  onChange={(e) => handleAllocationChange('need', Number(e.target.value))}
                  className="flex-1"
                />
                <Percent className="h-4 w-4 text-muted-foreground" />
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                <Input
                  ...
                  onChange={(e) => handleAllocationChange('want', Number(e.target.value))}
                  className="flex-1"
                />
                <Percent className="h-4 w-4 text-muted-foreground" />
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                <Input
                  ...
                  onChange={(e) => handleAllocationChange('investments', Number(e.target.value))}
                  className="flex-1"
                />
                <Percent className="h-4 w-4 text-muted-foreground" />
              </div>
        ...
        <Card className="bg-gradient-to-r from-primary/5 to-success/5 border-primary/20">
          <CardContent className="pt-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-primary">Complete Budget Breakdown</h3>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              <div className="p-3 bg-destructive/10 rounded-lg border border-destructive/20">
                ...
                <p className="text-sm text-muted-foreground">{budgetAllocation.need}%</p>
                <p className="text-lg font-bold">₹{allocatedAmounts.need.toLocaleString()}</p>
              </div>
              ...
              <div className="p-3 bg-warning/10 rounded-lg border border-warning/20">
                ...
                <p className="text-sm text-muted-foreground">{budgetAllocation.want}%</p>
                <p className="text-lg font-bold">₹{allocatedAmounts.want.toLocaleString()}</p>
              </div>
              ...
              <div className="p-3 bg-success/10 rounded-lg border border-success/20">
                ...
                <p className="text-sm text-muted-foreground">{budgetAllocation.savings}%</p>
                <p className="text-lg font-bold">₹{allocatedAmounts.savings.toLocaleString()}</p>
              </div>
              ...
              <div className="p-3 bg-primary/10 rounded-lg border border-primary/20">
                ...
                <p className="text-sm text-muted-foreground">{budgetAllocation.investments}%</p>
                <p className="text-lg font-bold">₹{allocatedAmounts.investments.toLocaleString()}</p>
              </div>
            </div>
            ...
            <div className="p-3 bg-background/50 rounded-lg">
              <p className="text-sm text-muted-foreground">
                <strong>Remaining Amount:</strong> ₹{(actualSalary - calculatedBudget).toLocaleString()} 
                ({(100 - budgetPercentage)}% - for emergency funds, extra savings, etc.)
              </p>
            </div>
          </CardContent>
        </Card>
